import { ReceitaCompat } from "../../domain/receitas.types";
...
function extrairHorario(valor?: string | null): string | null {
  if (!valor) return null;
  const match = String(valor).match(/(\d{2}:\d{2}:\d{2})/);
  return match?.[1] ?? null;
}

function extrairData(valor?: string | null): string | null {
  if (!valor) return null;
  const match = String(valor).match(/^(\d{4}-\d{2}-\d{2})/);
  return match?.[1] ?? null;
}

function ehSomenteHora(valor: string): boolean {
  return /^\d{2}:\d{2}(:\d{2})?$/.test(valor.trim());
}

function normalizarHora(hora: string): string {
  if (/^\d{2}:\d{2}$/.test(hora.trim())) return `${hora.trim()}:00`;
  return hora.trim();
}

function somarDias(yyyyMmDd: string, dias: number): string {
  const base = new Date(`${yyyyMmDd}T00:00:00`);
  if (Number.isNaN(base.getTime())) return yyyyMmDd;
...
function detectarCicloHorarios(horarios: string[]): number {
  if (horarios.length <= 1) return 1;
  const first = horarios[0];
  for (let i = 1; i < horarios.length; i++) if (horarios[i] === first) return i;
  return Math.max(1, new Set(horarios).size);
}

function normalizarDataHoraTomadas(raw: string[], dataBase: string | null): string[] {
  if (raw.length === 0) return [];
  const base = dataBase && /^\d{4}-\d{2}-\d{2}$/.test(dataBase) ? dataBase : null;
  if (!base) return raw;
  if (!raw.every((v) => ehSomenteHora(v))) return raw;
...
export class ReceitasService {
  ...
  private mapear(raw: Awaited<ReturnType<ReceitasRepository["buscarRecentes"]>>): ReceitaCompat[] {
    const saida: ReceitaCompat[] = [];
    for (const r of raw) {
      const paciente = Array.isArray(r.a004_pacientes) ? r.a004_pacientes[0] : r.a004_pacientes;
      const medicamentos: ReceitaCompat["medicamentos"] = [];
      ...
      if (medicamentos.length === 0) continue;
      saida.push({
  ...
  async porData(dataReceita: string): Promise<ReceitaCompat[]> {
    return this.mapear(await this.repository.buscarPorData(dataReceita));
  }

  async porIntervaloData(dataInicio: string, dataFim: string): Promise<ReceitaCompat[]> {
    return this.mapear(await this.repository.buscarPorIntervaloData(dataInicio, dataFim));
  }

  async porNomePaciente(nomePaciente: string): Promise<ReceitaCompat[]> {
    return this.mapear(await this.repository.buscarPorNomePaciente(nomePaciente));
  }

  async porCartaoSus(cartaoSus: string): Promise<ReceitaCompat[]> {
