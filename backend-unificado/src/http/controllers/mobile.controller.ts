...
import { mobilePayloadSchema, MobilePayload } from "../../contracts/mobile.payload";

let ultimoPayloadMobile: MobilePayload | null = null;

export class MobileController {
  postReceitas(req: Request, res: Response): void {
    const parsed = mobilePayloadSchema.safeParse(req.body);
    if (!parsed.success) {
      res.status(400).json({
  ...
  getUltimo(_req: Request, res: Response): void {
